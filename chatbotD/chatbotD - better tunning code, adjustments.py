...
STATE_SIZE = len(keywords) + 6 # < <- ajustment ( autom ) 
...
def build_model():
    m=Sequential([
       ...
       Dense(16,activation='relu') ,  # neurons
...
def text_to_state(question:str):
    ...
    state = []
    for kw in keywords:
        state.append(1 if kw in lowq else 0)


    state.append(1 if ("how many" in lowq and "february" in lowq and "leap year" in lowq) else 0)
    state.append(1 if ("days" in lowq and "february" in lowq and "leap year" in lowq) else 0)
    state.append(1 if ("warsaw" in lowq and "capital" in lowq) else 0)
    state.append(1 if ("capital" in lowq and "which country" in lowq) else 0)
    state.append(1 if ("language" in lowq and "czech republic" in lowq) else 0)
    state.append(1 if ("painting" in lowq and "ermine" in lowq and "located" in lowq) else 0)
...
def choose_action(state, forced_explore=False):

    global epsilon

    if forced_explore or np.random.rand() < epsilon:
        ...
        confidence = qvals[0][action] / (np.sum(qvals[0]) + 1e-5)

    return action, confidence

def train_batch(epochs=5):
    ...
    for i in range(len(batch)):
        if dones[i]:
            qvals[i][actions[i]] = rewards[i] * 10  # mocniejsza reakcja na feedback
        else:
            qvals[i][actions[i]] = rewards[i] + GAMMA * np.max(qnext[i])

    model.fit(states, qvals, epochs=epochs, verbose=0)
...
def auto_train(episodes=1000):
    ...
    for ep in range(episodes):
        ...
        action, confidence = choose_action(s)   
    ...
    while True:
        ...
        for (qtxt,corr) in training_data:
            if user_q.lower()==qtxt.lower():
                auto_rew= +10 if (corr==action) else -10
        ...
        if user_fb=='+':
            ...
            rew = auto_rew if auto_rew is not None else 0
            ...
            wrong_attempts[user_q] = wrong_attempts.get(user_q, 0) + (1 if rew < 0 else -wrong_attempts.get(user_q,0))

            forced_explore = wrong_attempts[user_q] >= 3
            action, confidence = choose_action(s, forced_explore=forced_explore)
        ...
        was_repeated = previous_responses.get(user_q) == ans
        previous_responses[user_q] = ans

        if ans in ["I don't know", "???"]:
        ...
        logging.info(
            ...
            f"eps={epsilon:.3f}, CONFIDENCE={confidence:.4f}, TIME={elapsed:.4f}s, "  # <-- NEW: logging confidence and elapsed time
            f"CPU=({cpu_before:.2f}%->{cpu_after:.2f}%), MEM=({mem_before:.2f}MB->{mem_after:.2f}MB), "
            f"answer='{ans}', lenResp={length_resp}, Repeated={was_repeated} "
        )
        ecount+=1
